# Kamran Steel Works — Estimate for practice-orders/order-4.txt (Window + Gate + Grill)

Draft — not yet reviewed/approved.

## 1. Rates (fetched live via `get_material_rate`, mwpbnp.com/pricing, 2026-08-19)
...
⚠️ **ASSUMPTION**: "Square Pipe"/"Rectangular Pipe" mapped to the site's "Pipe ReRolled = Square + Rectangle" category (as opposed to "Pipe Prime Rectangle", a separate, pricier line for some of the same nominal sizes) — the order doesn't name a brand/grade, so the plain ReRolled line was used.
...
⚠️ **ASSUMPTION**: 1/2" square bar per-foot rate (Rs 102.267570/ft) is not a scraped rate — it's derived from the live Rs 265/kg rate via the standard mild-steel weight formula (kg/m = 0.00785 × side_mm² → kg/ft → × Rs/kg). Shown at full precision so this line reconciles exactly with its stated cost.

⚠️ **ASSUMPTION**: Gate horizontal bars — order's item 4 ("1/2" Square Bar (Horizontal)") has no footage given; assumed 6 pcs @ 10'-0", based on the vertical-bar spacing pattern (22 pcs @ 7'-0").

⚠️ **ASSUMPTION**: 18G Sheet area — assumed full 10'x7' gate face (70 sq ft); size not stated in the order.

⚠️ **ASSUMPTION**: 3mm Glass (window item 4) — no pane size/count given, and the live pricing tool found no glass rate on the site (checked keyword "glass": no match). Excluded entirely rather than guessed.

**Excluded entirely** (no data to assume from, and no live rate available — checked "hinges", "lock", "handle", "aldrop": no matches on the site): hardware — hinges, lock, handle (window); hinges, aldrop, lock, handle (gate).
...
AWAITING OWNER-SET PROFIT % to finalize.
