...
def _price_value(rate_text: str) -> Optional[int]:
    """Pull the numeric Rs amount out of a logged rate string, e.g. 'Rs 1,790' -> 1790."""
    match = _FIRST_PRICE_RE.search(rate_text)
    if not match:
        return None
...
def _latest_per_keyword(entries: list[tuple[str, str, str]]) -> list[tuple[str, str, str]]:
    """Reduce entries to the most recent (date, keyword, rate) logged per keyword, in first-seen order."""
    latest: dict[str, tuple[str, str, str]] = {}
    for entry_date, keyword, rate in entries:
        latest[keyword] = (entry_date, keyword, rate)
...
def morning_brief() -> str:
    """
    Summarize the latest progress.md activity in a short, human-readable
    briefing (not a dump of the raw log).

    Looks at whatever was logged today first. Since check_and_log_rate only
    appends a line when a rate is new or has changed, "today's entries"
    means "materials whose rate changed today" -- each is compared against
    its previous logged rate to say whether it's brand new or moved up/down.
    If nothing was logged today, falls back to the most recent entry on
    file for each material.

    Returns:
        A 2-3 sentence plain-text summary, e.g. "Good morning! New rate
        found for MS pipe: Rs 1,790 (up from Rs 1,750)." or "Good morning!
        No new rate checks yet today. Latest on file: MS pipe at Rs 1,790
        (last logged 2026-08-18)."
    """
    entries = _read_log_entries()
    if not entries:
        return "Good morning! No rate checks have been logged yet."

    today = date.today().isoformat()
    today_entries = [e for e in entries if e[0] == today]

    if today_entries:
        sentences = []
        for entry_date, keyword, rate in today_entries:
            prior = [e for e in entries if e[1] == keyword and e[0] < today]
            if not prior:
                sentences.append(f"New rate found for {keyword}: {rate}.")
                continue
            old_price = _price_value(prior[-1][2])
            new_price = _price_value(rate)
            if old_price is not None and new_price is not None and old_price != new_price:
                direction = "up" if new_price > old_price else "down"
                old_display = _FIRST_PRICE_RE.search(prior[-1][2])
                old_display = old_display.group(0) if old_display else prior[-1][2]
                sentences.append(f"New rate found for {keyword}: {rate} ({direction} from {old_display}).")
            else:
                sentences.append(f"Checked {keyword} today: rate unchanged at {rate}.")
        return "Good morning! " + " ".join(sentences)

    latest = _latest_per_keyword(entries)
    parts = [f"{keyword} at {rate} (last logged {entry_date})" for entry_date, keyword, rate in latest]
    return "Good morning! No new rate checks yet today. Latest on file: " + "; ".join(parts) + "."
